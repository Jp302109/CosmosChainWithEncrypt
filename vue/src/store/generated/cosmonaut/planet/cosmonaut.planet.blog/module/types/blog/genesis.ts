...
const baseGenesisState: object = { sentPostCount: 0, timedoutPostCount: 0, postCount: 0, portId: '' }

export const GenesisState = {
  encode(message: GenesisState, writer: Writer = Writer.create()): Writer {
    for (const v of message.sentPostList) {
      SentPost.encode(v!, writer.uint32(50).fork()).ldelim()
    }
    if (message.sentPostCount !== 0) {
      writer.uint32(56).uint64(message.sentPostCount)
    }
    for (const v of message.timedoutPostList) {
      TimedoutPost.encode(v!, writer.uint32(34).fork()).ldelim()
    }
    if (message.timedoutPostCount !== 0) {
      writer.uint32(40).uint64(message.timedoutPostCount)
    }
